...
function _load(sourse, obj1, obj2, callback, end){
	...
	var loader = Preloader.create(function(type, val) {
			...
			switch (type) {
				...
				//加载完成后的动作break;
				if(obj1 || obj2){
					//console.log(obj2.parentNode.parentNode)
					if(obj1) obj1.parentNode.parentNode.style.display='none';
					if(obj2) obj2.parentNode.parentNode.style.display='none';
				}
				end && end();
				callback && callback();
				case "progress":
...
return：null;
*/
function reload(dataSrc, obj1, obj2, callback){
	//设置默认值
	dataSrc=dataSrc || '';
	obj1=obj1 || null;
	obj2=obj2 || null;
	...
	//判断图片类型是否为img，
	_load(arr, obj1, obj2, function(){
		for(var i=0; i<arr.length; i++){
			//console.log(aImg[i].tagName)
